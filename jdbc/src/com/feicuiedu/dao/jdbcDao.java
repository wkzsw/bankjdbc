...
public class jdbcDao<T> implements UserDao<T> {
	// 表的别名
	private static final String TABLE_ALIAS = "user_";
	...
	public void save(T t) throws Exception {
		...
		for (Field field : fields) {
			...
			if (field.isAnnotationPresent(Id.class)) {
				// getReadMethod()Class对象所表示的类或接口的指定公共成员方法
		...
		PreparedStatement ps = Dao.getConnection().prepareStatement(sql.toString());
		// 设置SQL参数占位符的值
		setParameter(fieldValues, ps, false);
		// 执行SQL
		ps.execute();
		Dao.release(ps, null);
		//// 得到类的简写名称getSimpleName
	...
	public void delete(Object id, Class<T> clazz) throws Exception {
		...
		if (!flag) {
			// throw new NotFoundAnnotationException(clazz.getName() + " object not found id
			// property.");
		}
	...
	private String getTableName(Class<?> clazz) {
		if (clazz.isAnnotationPresent(Entity.class)) {// 判断是否使用了某个注解
			Entity entity = clazz.getAnnotation(Entity.class);// 获取注解
			return entity.value();// 返回注解
		} else {
			// throw new (clazz.getName() + " is not Entity Annotation.");
		}
		return null;
